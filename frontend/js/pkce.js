...
export async function genererCodeChallenge(codeVerifier) {
  // crypto.subtle nécessite HTTPS. Fallback sur implémentation pure JS en HTTP.
  if (crypto.subtle) {
  ...
  return base64UrlEncode(sha256Pure(new TextEncoder().encode(codeVerifier)));
}

// SHA-256 pur JS (RFC 6234) — utilisé uniquement sur HTTP sans crypto.subtle
function sha256Pure(data) {
  ...
  new DataView(padded.buffer).setUint32(padded.length - 4, ml, false);
